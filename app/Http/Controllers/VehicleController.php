...
class VehicleController extends Controller
{
  ...
  //CREATE NEW VEHICLE 
    public function create_vehicle(Request $request){
      ...
      $dateofTireChange = date("Y-m-d", strtotime(date("Y-m-d", strtotime($vehicleTirePurchase)) . " + 3 year"));
      $dateofBatteryChange = date("Y-m-d", strtotime(date("Y-m-d", strtotime($vehicleBatteryPurchase)) . " + 3 year"));
      ...
      Tire::create([
          ...
          'date_replace' => $request->vehicleCondition == 'new' ? $dateofTireChange :  $request->vehicleTirePurchase,
      ]);
      Battery::create([
          'vehicle_id' => $vehicle->id,
          'brand_name'=>$request->vehicleBattery,
          'date_replace'=> $request->vehicleCondition == 'new' ? $dateofBatteryChange  : $request->vehicleBatteryPurchase,
      ]);
      OdoMeter::create([
          'vehicle_id' => $vehicle->id,
  ...
  //RETURN ODO RECORD OF THE VEHICLE
  ...
  //RETURN TIRE LOGS OF THE VEHICLE
